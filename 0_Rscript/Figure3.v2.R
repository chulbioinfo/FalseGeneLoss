...
zf_missing_compare_df <- read.csv(paste0(zf_path,"missingGenes/compare.missing.tsv"), sep="\t")
an_missing_compare_df <- read.csv(paste0(an_path,"missingGenes/compare.missing.tsv"), sep="\t")
pl_missing_compare_df <- read.csv(paste0(pl_path,"missingGenes/compare.missing.tsv"), sep="\t")
cl_missing_compare_df <- read.csv(paste0(cl_path,"missingGenes/compare.missing.tsv"), sep="\t")
...
species_list <- c("Zebra finch", "Platypus", "Anna's hummingbird", "Climbing perch")
...
all_missing_compare_df$platform <- ifelse(all_missing_compare_df$species %in% c("Zebra finch", "Platypus"), "Sanger-based", "Illumina-based")
all_missing_compare_df$platform <- factor(all_missing_compare_df$platform, levels=c("Sanger-based", "Illumina-based"))
head(all_missing_compare_df)
...
all_missing_compare_df$type <- ifelse(all_missing_compare_df$type == "intron", "intronic", all_missing_compare_df$type)
all_missing_compare_df$type <- ifelse(all_missing_compare_df$type == "noncoding_genes", "noncoding genes", all_missing_compare_df$type)
...
zf_missing_gene_df <- read.csv(paste0(zf_path,"missingGenes/ref.genes.not_aligned.bed"), sep="\t", col.names = missing_gene_colnames)
an_missing_gene_df <- read.csv(paste0(an_path,"missingGenes/ref.genes.not_aligned.bed"), sep="\t", col.names = missing_gene_colnames)
pl_missing_gene_df <- read.csv(paste0(pl_path,"missingGenes/ref.genes.not_aligned.bed"), sep="\t", col.names = missing_gene_colnames)
cl_missing_gene_df <- read.csv(paste0(cl_path,"missingGenes/ref.genes.not_aligned.bed"), sep="\t", col.names = missing_gene_colnames)
