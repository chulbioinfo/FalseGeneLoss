...
shared_theme <- function(my_col, y_breaks) {
  list(theme_pubr(),
    ...
    guides(linetype=guide_legend(ncol = 1, keyheight = unit(3, "mm")),
           color = guide_legend(ncol = 1, keyheight = unit(3, "mm")))
  )
}
...
fig.4.a.maker <- function(species_s, summary_df) {
  ...
  up3kb_plot <- ggplot(melt.upstream_3kb, aes(x =position_rename, y = value, color=CpG_n_variable)) + 
    ...
    geom_point(aes(group=CpG_n_variable, color=CpG_n_variable), size=0.4) +
    scale_x_continuous(breaks=c(1,15,30), labels=upstream_3kb_labels, name="Upstream (Kbp)",
                       sec.axis=sec_axis(~. + 0, name=" ", breaks = c(1), labels=c(" "))) + 
    coord_cartesian(ylim=c(0,80.5), xlim=c(0.8,30.5), expand=FALSE)+
    labs(color="Color", title="Upstream", x="Upstream (Kbp)",y="Percentage (%)") + 
    shared_theme(my_col2, y_breaks)
  
  down3kb_plot <- ggplot(melt.downstream_3kb, aes(x =position_rename, y = value, color=CpG_n_variable)) + 
    geom_line(aes(group=CpG_n_variable, color=CpG_n_variable), lwd=0.3) +
    geom_point(aes(group=CpG_n_variable, color=CpG_n_variable), size=0.4) +
    scale_x_continuous(breaks=c(1,15,30), labels=downstream_3kb_labels, name="Downstream (Kbp)",
                       sec.axis=sec_axis(~. + 0, name=" ", breaks = c(1), labels=c(" "))) + 
    coord_cartesian(ylim=c(0,80.5), xlim=c(0.8,30.5), expand=FALSE)+
    labs(color="Color", title="Downstream", x="Downstream (Kbp)") + 
    shared_theme(my_col2, y_breaks)
...
ggsave(fig4.a.zf_filepath, fig4.a.zf_plot, width=3, height=1.5) 
...
ggsave(fig4.a.an_filepath, fig4.a.an_plot, width=3, height=1.5) 
...
ggsave(fig4.a.pl_filepath, fig4.a.pl_plot, width=3, height=1.5)
...
ggsave(fig4.a.cl_filepath, fig4.a.cl_plot, width=3, height=1.5) 
...
fig.4.b.maker<- function(gc_n_missing_summary,species_s,gene_biotype){
  ...
  Fig4.d_g.plot <- ggplot(gc_n_missing_summary) + 
    ...
    geom_bar(data=subset(gc_n_missing_summary, gc_n_missing_summary$position %in% c(exon_order)), 
             ...
             alpha=0.5,width=3)+
    scale_colour_manual(name=" ",values=color_palette, labels=c("GC content (Missing)", "GC content (Present)", "Missing ratio")) + 
  ...
  if(gene_biotype %in% c("protein\ncoding", "lncRNA")){
    Fig4.d_g.plot <- Fig4.d_g.plot + scale_x_continuous(breaks=c(1,15,30, # upstream 3kb 
                                                                 36,46,56, # first, internal, last exons
                                                                 max(gc_n_missing_summary$num_position)-30,max(gc_n_missing_summary$num_position)-15,max(gc_n_missing_summary$num_position)), #downstream 3kb
                                                        labels=c("-3","-1.5","-0","F","I","L","+0","+1.5","+3"), expand=c(0,0))}
  else {
    Fig4.d_g.plot <- Fig4.d_g.plot + scale_x_continuous(breaks=c(1,15,30, # upstream 3kb 
                                                                 36, # internal exon
                                                                 max(gc_n_missing_summary$num_position)-30,max(gc_n_missing_summary$num_position)-15,max(gc_n_missing_summary$num_position)), #downstream 3kb
                                                        labels=c("-3","-1.5","-0","E","+0","+1.5","+3"), expand=c(0,0))}
  fig4.d_g.legend = gtable_filter(ggplotGrob(Fig4.d_g.plot), "guide-box")
  fig4.d_g.legend_path = "/disk1/juwan_d1/cat_rerun/1_Main_figures/Fig4/fig4.all.legend.png"
...
ggsave(fig.4.b.filepath, fig.4.b.plots, width=5.6, height=5.8)
