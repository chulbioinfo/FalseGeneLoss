...
script= ./stats/ #$VGP_PIPELINE/stats/
...
echo
echo "=== Scaffolds ==="
java -jar -Xmx2g ./faiCalcNGStats.jar $fasta.fai $gsize 1 > $asm.scaff.stats
...
if [ ! -e $asm.gaps ]; then
  java -jar -Xmx4g ./fastaGetGaps.jar $fasta $asm.gaps
fi
...
if [[ $num_gaps -gt 0 ]]; then
	...
	echo "=== Contigs ==="
		...
		java -jar -Xmx2g ./faiCalcNGStats.jar $asm.contigs.len $gsize > $asm.contigs.stats
	#fi

	echo
	echo "=== Gaps ==="
	#if [ ! -e $asm.gaps.stats ]; then
		awk '{print $NF"\t"$4}' $asm.gaps | java -jar -Xmx2g ./faiCalcNGStats.jar - $gsize > $asm.gaps.stats
	#fi
  #rm $asm.contigs.len
fi
#rm $asm.gaps
...
java -jar -Xmx2g ./lenCalcNGStats.jar $asm.p.len $gsize > $asm.p.stats
...
java -jar -Xmx2g ./lenCalcNGStats.jar $asm.contigs.p.len $gsize 1 > $asm.contigs.p.stats
echo "Contigs"
cat $asm.contigs.p.stats
echo

grep "scaffold_" $asm.gaps > $asm.gaps.p
java -jar -Xmx2g ./lenCalcNGStats.jar $asm.gaps.p $gsize 3 > $asm.gaps.p.stats
echo "Gaps"
...
if [ ! -e $asm.p.fasta ]; then
    ...
    java -jar -Xmx1g ./fastaExtractFromList.jar $asm.fasta $asm.p.list $asm.p.fasta
fi
echo

echo "=== Alt Stats ==="
grep -v "scaffold_" $fasta.fai > $asm.h.len
java -jar -Xmx2g ./lenCalcNGStats.jar $asm.h.len $gsize > $asm.h.stats
cat $asm.h.stats
echo
...
if [ ! -e $asm.h.fasta ]; then
    ...
    java -jar -Xmx2g ./fastaExtractFromList.jar $asm.fasta $asm.h.list $asm.h.fasta
fi
#END
